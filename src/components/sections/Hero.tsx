          ...
          <div className="flex flex-wrap gap-6 lg:gap-12 mb-8 lg:mb-10 fade-in">
            {STATS.map((stat, index) => (
              <div key={index} className="stat">
                <div className="text-4xl lg:text-5xl font-light text-white leading-none">{stat.value}</div>
                <div className="text-xs lg:text-sm text-gray-400 uppercase tracking-widest mt-2">{stat.label}</div>
              </div>
            ))}
          </div>
          <div className="flex flex-col sm:flex-row gap-4 items-start sm:items-center fade-in">
               <Button href="mailto:Scott@elyanlabs.ai?subject=Connect%20with%20Sophia" variant="primary">
            ...
            <Button href="#sophiacore" variant="secondary">
              <span className="text-sm uppercase tracking-wider">Explore Technology</span>
            </Button>
          </div>
        </div>
